...
class dspp {
  ...
  async _parse() {
    try {
      // Debug statement indicating the start of the parsing process
      ...
      // Iterate through each compose file for parsing
      for (let compose_file of compose_files || []) {
        // Log the name of the file being parsed
        logger.info('Parsing file:', compose_file);

        // Read the content of the compose file and prepare the body
        let body = env + readFileSync(compose_file);
        stack += `${body}\n---\n`;
        progress.tick();

        try {
          // Replace certain patterns in the body
          let ctx = { ...dict };
          let token = guid();
          body = body.replace(new RegExp('\\$\\$\\{', 'g'), token);
          body = Function('{ctx}', "return `" + body + "`;").call(null, { ctx });
          ...
          // Parse the body using laxParser
          let doc = laxParser(body);
          ...
          // Convert the parsed document to JavaScript object
          doc = doc.toJS({ maxAliasCount: -1 });
          // Deeply mix the parsed document into the 'out' object
          deepMixIn(out, doc);

          // Set the source file for each item in services, configs, and tasks
          for (let obj of ['services', 'configs', 'tasks']) {
            for (let id of Object.keys(doc[obj] || {}))
              out[obj][id][SOURCE_FILE] = compose_file;
          }
          ...
          console.error('\n', 'Parsing failure in', compose_file);
          logger.error('Error occurred during parsing in file', compose_file, 'Error:', err);
          throw err;
        }
      }

      // Sort 'out' object by keys and replace environment variables
      out = sortObjByKey(out);
      out = walk(out, v => replaceEnv(v, { ...out, stack_name }));

      // Function to process environment files
      let processEnvFile = async (obj) => {
        if (!obj.env_file)
          return obj;
      ...
      // Process tasks and services in the 'out' object
      for (let [task_name, task] of Object.entries(out.tasks || {}))
        out.tasks[task_name] = walk(task, v => replaceEnv(v, { ...task, task_name, service_name: task_name }));
      ...
      // Initialize a progress bar for computing configs
      progress = new ProgressBar('Computing (:total) configs [:bar]', { ...this.progressOpts, total: Object.keys(out.configs || {}).length });

      // Iterate through different passes for computing configs
      for (let skip of [
        // 1st pass: skip serialized
        config => !!config.external || !!config.format || !!config.bundle,
        // 2nd pass: skip bundle
        config => !!config.external || !!config.file || !!config.bundle,
        // 3rd pass: skip processed
        config => !!config.external || !!config.file,
      ]) {
        // Get the entries of 'out.configs' object
        let configs = Object.entries(out.configs || {});

        // Iterate through each config for computation
        for (let [config_name, config] of configs) {
          // Skip configs based on the criteria defined in 'skip' function
          if (skip(config))
            continue;
          ...
          try {
            // Initialize an empty array for storing config lines
            config_map[config_name] = [];
            // Iterate through lines obtained from 'cas.config'
            for await (let line of cas.config(config_map, config_name, config, config[SOURCE_FILE])) {
              let { cas_path, cas_name, trace } = line;
              // Push the line into the config map
              config_map[config_name].push(line);
              // Update 'out.configs' with the new config
              out.configs[cas_name] = { name: config.name, file: cas_path, [CONFIG_NAME]: config_name };
              // If there's a trace, update 'x-trace' in 'out.configs'
              if (trace)
                out.configs[cas_name]['x-trace'] = trace;
            }
            // Delete the original config from 'out.configs'
            delete out.configs[config_name];
          } catch (err) {
            // Log an error message with the file name
            console.error('\n', 'Error occurred during computing configs for', config_name, 'in file', config[SOURCE_FILE]);
            logger.error('Error occurred during computing configs for', config_name, 'Error:', err);
            throw err;
          }
        }
      }

      // Process volumes and clean up 'out' object
      let volumes_map = {};
      let volumes = Object.entries(out.volumes || {});
      ...
      deepWalk(out, (v) => {
        // Deep mixing for non-alias merges
        if (typeof v !== 'object' || v === null)
          return v;
      ...
      // Iterate through all services and tasks to update their configs
      for (let obj of Object.values({ ...out.services, ...out.tasks })) {
        if (!obj.configs)
          continue;
        let base = obj.configs || [];
      ...
      // Initialize variables for stack GUID and return the result
      let stack_guid = md5(stack);

      return { out: { services: {}, tasks: {}, configs: {}, volumes: {}, secrets: {}, networks: {}, ...out }, stack_guid, cas };
    } catch (error) {
      // Log an error message and rethrow the error
      console.error('\n', 'Error occurred during parsing:', error);
      logger.error('Error occurred during parsing:', error);
      throw error;
    }
  }
