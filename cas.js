...
let lastProcessedConfig = '';

async function showProgress(config_name, filename) {
  process.stdout.clearLine();
  process.stdout.cursorTo(0);
  process.stdout.write(`Processing config: ${config_name}, File: ${filename}`);
  lastProcessedConfig = `Processing config: ${config_name}, File: ${filename}`;
}
...
class Cas {
  ...
  async * config(config_map, config_name, config, source_file, target = "") {
    ...
    // Notify progress dynamically
    process.stderr.clearLine(0);
    process.stderr.write(`\rProcessing config: ${config_name}`);
  }
}
